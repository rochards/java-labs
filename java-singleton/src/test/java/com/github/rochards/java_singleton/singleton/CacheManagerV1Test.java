...
class CacheManagerV1Test {

    @Test
    void shouldSaveAndRetrieveAValue() {
        var cache = CacheManagerV1.getInstance();
    ...
    void shouldVerifyItIsASingleton() {
        var cache1 = CacheManagerV1.getInstance();
        var cache2 = CacheManagerV1.getInstance();

        assertSame(cache1, cache2);
    }

    @Test
    void shouldExposeThatLazyInitializationIsNotThreadSafe() throws Exception {
        // The sleep inside getInstance() widens the race window so multiple threads can observe instance == null.
        CacheManagerV1.resetInstance();
        ...
        var instances = Collections.synchronizedSet(new HashSet<CacheManagerV1>());
            ...
            for (int i = 0; i < threadCount; i++) {
                executor.submit(() -> { // Schedules one task that races to call getInstance().
                    try {
                        ...
                        instances.add(CacheManagerV1.getInstance()); // Calls the unsynchronized lazy initializer.
        ...
        assertTrue(instances.size() > 1, "Expected the race to create more than one instance.");
    }
}
